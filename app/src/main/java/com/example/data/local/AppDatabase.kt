...
@Database(
    entities = [Exercise::class, WorkoutTemplate::class, WorkoutSession::class, CardioSession::class,
        UserProfile::class, BodyMeasurement::class, ProgressPhoto::class, ExercisePerformanceTarget::class,
        com.example.data.model.MealLog::class, com.example.data.model.UserMedal::class],
    version = 8,
    exportSchema = true
)
...
abstract class AppDatabase : RoomDatabase() {
    ...
    private class DatabaseCallback(private val scope: CoroutineScope) : RoomDatabase.Callback() {
        override fun onCreate(db: SupportSQLiteDatabase) {
            super.onCreate(db)
            INSTANCE?.let { database -> scope.launch(Dispatchers.IO) { populateInitialData(database.fitnessDao()) } }
        }
