...
data class RoutePoint(val latitude: Double, val longitude: Double, val timestampMillis: Long, val accuracyMeters: Float? = null, val speedMetersPerSecond: Float? = null, val altitudeMeters: Double? = null)

@JsonClass(generateAdapter = true)
data class ActivitySplit(val kilometer: Int, val elapsedSeconds: Int, val movingSeconds: Int, val paceSecondsPerKm: Int, val elevationGainMeters: Double = 0.0)

@JsonClass(generateAdapter = true)
@Entity(tableName = "cardio_sessions", indices = [Index(value = ["dateEpochDay"])])
